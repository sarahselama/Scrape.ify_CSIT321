...
# \- (Optional) \*\*OpenAI API key\*\* if you wire an LLM behind the chatbot: set environment var `OPENAI\_API\_KEY`
...
# \- \*\*Custom scraper\*\*: go to \*\*Custom Scrape\*\* → provide URLs and fields → submit → a ready‑to‑edit Python script (`custom\_scrape.py`) is generated for you.
...
# \- `OPENAI\_API\_KEY` — only if you integrate an LLM for chatbot answers beyond rules/intents.

# \- `FLASK\_ENV=development` — enables debug mode during development.
