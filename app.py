...
@app.route('/chatbot')
def chatbot():
    return render_template('ai_chat_bot.html')
...
@app.route('/pro')
def pro():
    return render_template('pro_service.html')

@app.route('/custom-scrape')
def custom_scrape():
    return render_template('custom-bot.html')


@app.route('/generate-scrape', methods=['POST'])
def generate_scrape():
    product_name = request.form['productName']
    # Retrieve URLs from the form, expecting URLs to be separated by newline characters.
    urls = request.form['urls'].split('\n')
    selected_details = request.form.getlist('details')

    # Script beginning with function definitions
    script_content = """
from bs4 import BeautifulSoup as bs
import requests
import pandas as pd
...
"""
    ...
    # Dynamically add the selected details to the script
    if "title" in selected_details:
        script_content += "    product_details['title'] = get_title(soup)\n"
    if "price" in selected_details:
        script_content += "    product_details['price'] = get_price(soup)\n"
    if "rating" in selected_details:
        script_content += "    product_details['rating'] = get_rating(soup)\n"
    if "reviews" in selected_details:
        script_content += "    product_details['reviews'] = get_review_count(soup)\n"
    if "availability" in selected_details:
        script_content += "    product_details['availability'] = get_availability(soup)\n"

    # Complete the product details extraction and include user URLs
    script_content += "    return product_details\n\n"

    # Example usage part
    script_content += "# Example usage\nif __name__ == '__main__':\n"
    script_content += "    urls = ["
    script_content += ",\n    ".join(f'"{url}"' for url in urls)
    script_content += "]\n"
    script_content += "    for url in urls:\n"
    script_content += "        print(scrape_website(url))\n"

    # Encode and prepare the script for download
    script_bytes = script_content.encode('utf-8')
    script_file = io.BytesIO(script_bytes)
    script_file.name = "custom_scrape.py"
    script_file.seek(0)

    return send_file(script_file, mimetype="text/x-python", as_attachment=True, download_name="custom_scrape.py")
...
intents = parse_intents('intents.json')
...
def send_message():
    ...
    for intent in intents['intents']:
        if user_input == intent['pattern']:
            response_text = intent.get('response', '')
            response_options = intent.get('options', [])
            
            # Prepare the sub-options to send
            if response_options:
                response_to_send['options'] = response_options
            else:
                response_to_send['response'] = response_text
                response_to_send['url'] = intent.get('url', '')

            break

    if not response_to_send:
        response_to_send = {"response": "I'm not sure how to help with that."}
...
def search():
    if request.method == 'POST':
        product_input = request.form['product_name']
        ...
        output_filename = 'ScrapeIFY_Results.xlsx'
